  ...
  const handleOpenPolicyUrl = async (url: string, label: string) => {
    try {
      ...
      if (!canOpen) {
        Alert.alert(
          "Unable to open link",
          `Could not open ${label}. Please try again later.`,
        );
        return;
      }
      ...
      <SafeAreaView className="flex-1 justify-between">
        ...
        <View className="px-8 pb-4">
          ...
          <View className="flex-row justify-center items-center gap-4 mb-5">
            {/* GOOGLE btn */}
            <Pressable
              className="size-20 rounded-2xl bg-surface/90 items-center justify-center active:scale-95 border border-border"
            ...
            {/* APPLE btn */}
            <Pressable
              className="size-20 rounded-2xl items-center justify-center active:scale-95 bg-surface/90 border border-border"
              style={({ pressed }) => ({ opacity: pressed ? 0.85 : 1 })}
              disabled={isLoading}
              accessibilityRole="button"
